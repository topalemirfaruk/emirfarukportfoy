...
export default class Decor {
    ...
    createRaycaster() {
        window.addEventListener('mousedown', (event) => {
            ...
            if (intersects.length > 0) {
                // Find the first object that has a registered hitbox action
                const hit = intersects.find(intersect => this.hitboxes && this.hitboxes[intersect.object.name]);
                if (hit) {
                    const hb = this.hitboxes[hit.object.name];
                    if (hb) {
                        hb.action();
                    }
                }
    ...
    createComputerHitbox() {
        ...
        // Power Button Hitbox removed as per user request
    }
